...
def generate_launch_description():
    ...
    ap = argparse.ArgumentParser(prog='ros2 launch choirbot taskassignment.launch.py')
    ...
    table = Node(
            package='choirbot_examples', node_executable='choirbot_table', output='screen',
    ...
    for i in range(N):
        ...
        list_description.append(Node(
            package='choirbot_examples', node_executable='choirbot_task_guidance', output='screen',
            node_namespace='agent_{}'.format(i),
            prefix=['xterm -hold -e'],
            parameters=[{'agent_id': i, 'N': N, 'in_neigh': in_neighbors, 'out_neigh': out_neighbors}]))

        list_description.append(Node(
            package='choirbot_examples', node_executable='choirbot_planner', output='screen',
            node_namespace='agent_{}'.format(i),
            # prefix=['xterm -hold -e'],
            parameters=[{'agent_id': i}]))
        
        list_description.append(Node(
            package='choirbot_examples', node_executable='choirbot_controller', output='screen',
            node_namespace='agent_{}'.format(i),
            # prefix=['xterm -hold -e']
            parameters=[{'agent_id': i}]))
